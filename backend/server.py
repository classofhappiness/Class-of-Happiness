...
# ── CREATURE SUBMISSIONS (photo evolution, teacher/parent approval, World Gallery) ──

@api_router.post("/creature-submissions")
async def submit_creature_photo(request: Request):
    """Student submits a photo of their creature at a given stage — goes to pending review."""
    user = await get_current_user(request)
    if not user:
    ...
    if not student_id or not photo_b64:
        raise HTTPException(status_code=400, detail="student_id and photo are required")

    student_r = supabase.table("students").select("*").eq("id", student_id).execute()
    student = (student_r.data or [{}])[0]
    teacher_id = student.get("teacher_id")

    import base64, uuid as uuidlib
    file_id = str(uuidlib.uuid4())
    file_bytes = base64.b64decode(photo_b64.split(",")[-1])
    path_in_bucket = f"submissions/{student_id}/{file_id}.jpg"
    supabase.storage.from_("creature-images").upload(path_in_bucket, file_bytes, {"content-type": "image/jpeg"})
    photo_url = supabase.storage.from_("creature-images").get_public_url(path_in_bucket)

    country = None
    if teacher_id:
        admin_r = supabase.table("users").select("school_admin_id").eq("user_id", teacher_id).execute()
        school_admin_id = (admin_r.data or [{}])[0].get("school_admin_id")
        if school_admin_id:
            profile_r = supabase.table("school_profiles").select("country").eq("user_id", school_admin_id).execute()
            if profile_r.data:
                country = profile_r.data[0].get("country")
    ...
    result = supabase.table("creature_submissions").insert(record).execute()
    return result.data[0] if result.data else record


@api_router.get("/creature-submissions/pending")
async def get_pending_submissions(request: Request):
    """Teacher or parent sees submissions awaiting their review."""
    user = await get_current_user(request)
    if not user or user.get("role") not in ["teacher", "parent", "admin", "school_admin", "superadmin"]:
        raise HTTPException(status_code=403, detail="Access required")

    user_id = user.get("user_id")
    if user.get("role") == "teacher":
        result = supabase.table("creature_submissions").select("*").eq("teacher_id", user_id).eq("status", "pending").execute()
    elif user.get("role") == "parent":
        family_r = supabase.table("family_members").select("student_id").eq("user_id", user_id).execute()
        student_ids = [f["student_id"] for f in (family_r.data or []) if f.get("student_id")]
        if not student_ids:
            return []
        result = supabase.table("creature_submissions").select("*").in_("student_id", student_ids).eq("status", "pending").execute()
    else:
        result = supabase.table("creature_submissions").select("*").eq("status", "pending").execute()
    return result.data or []


@api_router.put("/creature-submissions/{submission_id}")
async def review_submission(submission_id: str, request: Request):
    """Approve or reject a pending creature photo submission."""
    user = await get_current_user(request)
    if not user:
        raise HTTPException(status_code=403, detail="Login required")
    body = await request.json()
...
@api_router.get("/creature-gallery/world")
async def get_world_gallery():
    """Aggregated, anonymised creature counts by country. No student data returned — ever."""
    result = supabase.table("creature_submissions").select("country").eq("status", "approved").execute()
    counts = {}
    for row in (result.data or []):
        c = row.get("country")
        if c:
    ...
    MIN_THRESHOLD = 5
    return [{"country": c, "count": n} for c, n in counts.items() if n >= MIN_THRESHOLD]
