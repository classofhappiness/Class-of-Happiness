...
async def _unhandled_exception_handler(request: Request, exc: Exception):
    ...
    500 and the real error message, which is genuinely misleading to debug from the
    frontend console. This handler runs inside CORSMiddleware's wrapping, so its JSON
    response gets real CORS headers attached like any normal response — the browser now
    sees an actual 500 with a body, not a fake CORS failure. HTTPExceptions are unaffected
    (FastAPI's own handler for those already runs first and already gets CORS headers)."""
