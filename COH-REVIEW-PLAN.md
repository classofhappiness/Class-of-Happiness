...
### A7. Colour drift — systemic, worse than "app vs. portal," no central fix point *(Section 7 #3/#4)* — 🟡 IN PROGRESS 2026-08-20 — infrastructure + highest-traffic files done, ~32 files remain
...
**Explicitly not done — remaining scope for next pass**, re-confirmed via fresh grep after this fix (not the original review's now-stale count): `admin/dashboard.tsx`, `auth/callback.tsx`, `auth/forgot-password.tsx`, `kiosk/index.tsx`, `parent/alerts.tsx`, `parent/checkin.tsx`, `parent/dashboard.tsx`, `parent/family-member-stats/[id].tsx`, `parent/family-strategies.tsx`, `parent/linked-child/[id].tsx`, `parent/my-wellbeing.tsx`, `parent/resources.tsx`, `parent/strategies.tsx`, `parent/widget.tsx`, `profiles/create.tsx`, `profiles/edit.tsx`, `settings.tsx`, `student/select.tsx`, `student/strategies.tsx`, `subscription/index.tsx`, `subscription/success.tsx`, `teacher/alerts.tsx`, `teacher/bulk-checkin.tsx`, `teacher/checkin.tsx`, `teacher/classrooms.tsx`, `teacher/resources.tsx`, `teacher/student-detail.tsx`, `teacher/students.tsx`, `teacher/widget.tsx`, `src/components/CelebrationOverlay.tsx`, `src/components/CreatureManagement.tsx`, `src/components/EmotionColourLoader.tsx`, `src/components/FeelingsButton.tsx` (~32 files). Also still untouched: the backend Python dict (CREATURES model + PDF generators in `server.py` still hardcode the old palette), and `portal100.html`'s own drifted functions (`renderTeacherAlerts`, the strategy zone picker, the school-sharing button — see Section 6 #8 in the original review). The fix pattern for each remaining frontend file is now mechanical: import `EMOTION_COLOURS` from `../../src/constants/emotionColours` (adjust relative path per file depth), delete the local hardcoded copy, reference `EMOTION_COLOURS.<zone>` in its place — same as the 4 files done here.
...
### A16. Creature submission code accepted no real validation until after photo upload — ✅ DONE 2026-08-19, deploy blocked by a Railway platform incident
...
**Verified:** diff reviewed before applying (isolated to the code-entry step and the new endpoint); `python3 -m py_compile` and `tsc --noEmit` both pass clean, no errors at all in either changed file (not even pre-existing ones). Committed and pushed (`cab54ea`). **Live verification not completed tonight** — Railway is reporting a platform-wide incident (deploys stuck in "Sleeping · Queued", confirmed unrelated to this code), so the new endpoint couldn't be exercised against production. Re-verify live once Railway recovers: call `GET /creatures/validate-code/{code}` with a known-good and a known-bad code and confirm both the code-entry screen and the final submit flow behave correctly.

### A17. Admin PIN entry fails after a fast password→unlock sequence — race condition, exposed (not caused) by today's L1 fix. Plan only, **not applied tonight** — deliberately deferred, touches auth
...
9. **A7** — 🟡 IN PROGRESS 2026-08-20 — shared constants file created, ZoneButton + 3 highest-traffic files fixed and pushed (`730f500`). ~32 frontend files + backend Python dict + portal.html remain, mechanical follow-on — see A7 section for the exact file list.
...
19. **A16** — ✅ DONE 2026-08-19 — creature submission code now validated before the full flow, not after. Committed and pushed; live verification blocked tonight by a Railway platform incident, unrelated to this code — re-verify once Railway recovers.
20. **A17** — plan written, **not applied** — admin PIN race condition, deliberately deferred to do with a fresh head since it touches auth. Re-verify the full L1 fix alongside this one when it's tackled.
