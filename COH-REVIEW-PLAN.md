...
**Real problem found by real-device listening, 2026-08-19 — Portuguese content/clip_key mismatch, distinct from the filename-pairing work above:** Jono reported the actual *spoken content* on several Portuguese helper clips doesn't match their clip_key/label — not a bad recording, a mispairing. By ear: Blue all 6 wrong, Green 1&2 correct but 3-6 wrong, Yellow 1 correct but 2-6 wrong, Red all 6 correct (including `red_5`, confirmed above). Investigated whether this was introduced by this session's own reorg work — ruled out: the Storage move API only renames, cannot alter file bytes, and re-checking every PT translation used for the mapping confirmed the semantic pairing was linguistically correct. Checked original upload timestamps (preserved through the move) as a possible ordering signal — dead end, all 20 files were uploaded in a ~2-second bulk window, no usable sequence in that window. **Conclusion: the mismatch predates this session's reorg — the files were already misaligned with their actual spoken content before being uploaded/renamed.** No file-metadata signal can resolve which content actually belongs where; this needs Jono to listen to each wrong slot and report what it actually says, so the correct re-pairing (not re-recording — content itself is fine, per Jono) can be applied. **Still open, waiting on that by-ear data.**
