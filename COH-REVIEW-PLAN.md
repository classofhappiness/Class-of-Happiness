...
**Still open:** `red_5`/`Pedir_ajudar` match not yet explicitly confirmed by Jono (low-stakes, same category as the already-confirmed `blue_2`). `DEFAULT_HELPERS["blue"][1]["name"]` (and the per-language `HELPERS_PT`/`ES`/`FR`/`DE`/`IT` equivalents) still read "Warm Drink" — needs updating to "Drink Water" to match the real recorded audio and Jono's current naming; not yet done, timing to be decided.
