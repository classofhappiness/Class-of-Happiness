// Real feature Aug 22: shared ISO-3166-1 alpha-2 country list, built to resolve a real
// format mismatch found while scoping the submission-time country picker (item 1/item 6
// review) - the only other country picker in the app, COUNTRY_FLAGS in admin/dashboard.tsx,
// stores full country names ("Portugal"), but the creature system's own `country` field is
// already an ISO-2 code end to end (server.py defaults to "PT", not "Portugal", and the
// country-of-origin/anonymity-threshold feature already compares codes). This file is the
// canonical ISO-2 list for creature submissions - NOT a replacement for COUNTRY_FLAGS, which
// is a separate, smaller, name-based list for a different feature (school profile setup)
// that was out of scope to touch here.
//
// Flags are computed from the code rather than hand-listed, so there's no risk of a
// transcription mistake across ~195 entries - a flag emoji is just two Unicode Regional
// Indicator Symbols, one per letter.
...
export const COUNTRIES: CountryOption[] = [
  ...
  { code: 'ES', name: 'Spain' }, { code: 'LK', name: 'Sri Lanka' }, { code: 'KN', name: 'St Kitts and Nevis' },
  { code: 'LC', name: 'St Lucia' }, { code: 'VC', name: 'St Vincent and the Grenadines' }, { code: 'SD', name: 'Sudan' },
